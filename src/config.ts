...
export default {
  ...
  commands: <{[operatingSystem: string]: Commands}> {
    unix: {
      ...
      deployService: "cd ${deployFolder} && git clone --depth 1 --single-branch --branch ${repoTag} ${repoUrl} ${serviceName} && cd ${serviceName} && npm i --omit=dev --no-save",
      ...
      updateService: "cd ${deployFolder}/${serviceName} && git fetch --depth 1 origin ${repoTag} && git reset --hard FETCH_HEAD && npm i --omit=dev --no-save",
