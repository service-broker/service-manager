...
export default {
  ...
  commands: <{[operatingSystem: string]: Commands}> {
    unix: {
      ...
      deployService: "cd ${deployFolder} && git clone --single-branch --branch ${repoTag} ${repoUrl} ${serviceName} && cd ${serviceName} && npm i --only=prod --no-save",
      ...
      updateService: "cd ${deployFolder}/${serviceName} && git fetch origin ${repoTag} && git reset --hard origin/${repoTag} && npm i --only=prod --no-save",
